...
def get_name(soup):
	...
	td_headers = ["t1sa1 t1sb2", "t2sa1 t2sb2", "t3sa1 t3sb2"]

	generated_names = [td.contents[0] for td 
					   in soup.find_all("td", {"headers":td_headers})]
...
def base_link_ops(base_link):
	...
	td_headers = ["t1sa1 t1sb1", "t2sa1 t2sb1", "t3sa1 t3sb1"]

	links = generate_links(td_headers, soup)
	codes = get_code(td_headers, soup)
	names = get_name(soup)
...
def create_header(soup):
	...
	td_headers = ["t1sa1 t1sb2", "t2sa1 t2sb2"]

	# The empty class attribute fixes problem with empty table row.
	party_name = soup.find_all("td", {"class":"", "headers":td_headers})
...
def get_votes(soup):
	...
	td_headers = ["t1sa2 t1sb3", "t2sa2 t2sb3"]

	party_votes = soup.find_all("td", {"class":"cislo", "headers":td_headers})
...
def process_links(generated_link, codes, names):
	"""Scrapes and parses generated links.
	Takes the base_link_ops function returns as arguments."""
...
def main():
	...
	proc_links = process_links(ops_return[0], 
							   ops_return[1], 
							   ops_return[2]
							   )
